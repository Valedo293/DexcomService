...
def reset_allarmi():
    print("[DEBUG] Reset allarmi")
    for k in allarmi_attivi:
        allarmi_attivi[k] = False

def valuta_eventi(storico):
    if len(storico) < 3:
        print("[DEBUG] Storico troppo corto per valutare eventi.")
        return
    ...
    print(f"[DEBUG] Valori storici: {valori}")
    print(f"[DEBUG] Trend storici: {trend}")
    ...
    if not allarmi_attivi["2_stabili"]:
        ...
        print("[DEBUG] Allarme 1 già attivo")
    ...
    if not allarmi_attivi["stabile_salita"]:
        ...
        print("[DEBUG] Allarme 2 già attivo")
    ...
    if not allarmi_attivi["discesa_ripida"]:
        ...
        print("[DEBUG] Allarme 3 già attivo")
    ...
    if not allarmi_attivi["3_stabili_discesa"]:
        ...
        print("[DEBUG] Allarme 4 già attivo")
...
while True:
    try:
        ...
        # Reset allarmi se il valore esce da tutti i range di controllo
        if storico and not any([
            70 <= storico[-1]["valore"] <= 140,
            80 <= storico[-1]["valore"] <= 160,
            80 <= storico[-1]["valore"] <= 100
        ]):
            reset_allarmi()
